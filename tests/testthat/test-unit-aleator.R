...
test_that("aleator_seed_env produces reproducible results", {
  r1 <- aleator_seed_env(42, rnorm(5))
  r2 <- aleator_seed_env(42, rnorm(5))
  expect_identical(r1, r2)
})

test_that("aleator_seed_env isolates seed state", {
  set.seed(99)
  before <- runif(1)

  # This should not affect the outer seed

  aleator_seed_env(42, rnorm(100))
...
test_that("aleator_seed_env rejects non-numeric seed", {
  expect_error(aleator_seed_env("abc", 1 + 1))
})

test_that("aleator_convergence_check passes with good diagnostics", {
  result <- aleator_convergence_check(
...
test_that("aleator_convergence_check fails on high R-hat", {
  result <- aleator_convergence_check(
...
test_that("aleator_convergence_check fails on low ESS", {
  result <- aleator_convergence_check(
...
test_that("aleator_convergence_check custom thresholds", {
  # Stricter thresholds
  result <- aleator_convergence_check(
...
test_that("aleator_param_recovery works with linear model", {
...
test_that("aleator_param_recovery rejects missing column names", {
  expect_error(
...
test_that("aleator_param_recovery rejects unnamed params", {
  expect_error(
    aleator_param_recovery(
      true_params = c(1, 2),
